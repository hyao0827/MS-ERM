...
def pre_process(path, y_n):
    ...
    data_3D = data_3D.reshape(-1, 4, 8, 9).reshape(4800, 4, -1).reshape(40, -1, 4, 72)
    print("final data shape:", data_3D.shape)
    return data_3D, arousal_labels, valence_labels


if __name__ == '__main__':
    dataset_dir = r".\data_result\all_0.5/"
    use_baseline = "yes"
    if use_baseline == "yes":
        result_dir = r".\data_result\with_base_0.5/"
        if os.path.isdir(result_dir) == False:
            os.makedirs(result_dir)
    else:
        result_dir = r".\data_result\without_base_0.5/"
        if os.path.isdir(result_dir) == False:
            os.makedirs(result_dir)
