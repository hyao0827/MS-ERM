    ...
    for trial in range(40):
        ...
        for channel in range(32):
            ...
            for index in range(120):
                DE_theta = np.append(DE_theta, compute_DE(theta[index * 64:(index + 1) * 64])-base_theta_DE)
                DE_alpha = np.append(DE_alpha, compute_DE(alpha[index * 64:(index + 1) * 64])-base_alpha_DE)
                DE_beta = np.append(DE_beta, compute_DE(beta[index * 64:(index + 1) * 64])-base_beta_DE)
                DE_gamma = np.append(DE_gamma, compute_DE(gamma[index * 64:(index + 1) * 64])-base_gamma_DE)
...
def get_labels(file):
    ...
    final_arousal_labels = final_arousal_labels.reshape(40, -1)
    final_valence_labels = final_valence_labels.reshape(40, -1)
...
if __name__ == '__main__':
    dataset_dir = r".\data\matlabPREPROCESSED/"

    result_dir = r".\data_result\all_0.5/"
