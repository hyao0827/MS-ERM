...
class CrossValidation:
    ...
    def load_all_subject(self, subs):
        data_all = []
        label_all = []
        for sub in subs:
            save_path = os.getcwd()
        ...
        print('>>> Data:{} Label:{}'.format(data_all.shape, label_all.shape))
    ...
    def dependent(self, subject=[], fold=10, reproduce=False):
        """
        ...
        for sub in subject:
            ...
            for idx_fold, (index_train, index_test) in enumerate(kf.split(data)):
                ...
                if reproduce:
                    ...
                    acc_test, pred, act, f = test(args=self.args, data=data_test, label=label_test,
                                               reproduce=self.args.reproduce,
                                               subject=sub, fold=idx_fold)
        ...
        print('Final: test mean ACC:{} std:{} f1:{}'.format(mACC, std, mF1))
        ...
        self.log2txt(results)
        results = 'independent experiment results acc= {}'.format(tta)
        self.log2txt(results)
        results = 'independent experiment results f1= {}'.format(ttf)
        self.log2txt(results)

    def independence(self, subject=[], fold=10, reproduce=False):
        """
                this function achieves n-fold cross-validation
                :param subject: how many subject to load
                :param fold: how many fold
        ...
        data, label = self.load_all_subject(subject)
        data = data.reshape(-1, 1, 4, 72)
        label = label.reshape(-1)
        va = Averager()
        va_val = Averager()
        ...
        for idx_fold, (index_train, index_test) in enumerate(kf.split(data)):
            ...
            data_train, label_train, data_val, label_val = self.split_balance_class(
                data=data_train, label=label_train, train_rate=0.9, random=True
            )
            ...
            if reproduce:
                ...
                acc_val, test_loss = train(args=self.args,
                                data_train=data_train,
                                label_train=label_train,
                                data_val=data_val,
                                label_val=label_val,
                                ...
                                fold=idx_fold)
                # test the model on testing data
                acc_test, pred, act, f, c_m1= test(args=self.args, data=data_test, label=label_test,
                                           reproduce=self.args.reproduce,
                                           subject='all', fold=idx_fold)
            ...
            va_val.add(acc_val)
            va.add(acc_test)
            ssacc.append(acc_test)
            ssf1.append(f)
            preds.extend(pred)
            acts.extend(act)
            preds_trial.extend(pred_trial)
            acts_trial.extend(act_trial)
            fold_result = 'fold={} con_matrix1={}'.format(idx_fold, c_m1)
            self.log2txt(fold_result)
            fold_result = 'fold={} test_loss={}'.format(idx_fold, test_loss)
            self.log2txt(fold_result)

        tva.append(va_val.item())
        acc, f1, c_m2 = get_metrics(y_pred=preds, y_true=acts)
        acc_trial, f1_trial, c_m3 = get_metrics(y_pred=preds_trial, y_true=acts_trial)

        tta.append(acc)
        ttf.append(f1)
        tta_trial.append(acc_trial)
        ttf_trial.append(f1_trial)
        result = 'acc: {},f1: {}'.format(tta[-1], ttf[-1])
        self.log2txt(result)
        ...
        print('Final: test mean ACC:{} f1:{} std:{}'.format(mACC, mF1, std))
        print('Final: val mean ACC:{} std:{}'.format(mACC_val, std_val))
        results = 'test mAcc={} mF1={} val mAcc={}'.format(mACC, mF1, mACC_val)
        self.log2txt(results)
        results = 'test mAcc={} mF1={} (trial-wise)'.format(mACC_trial, mF1_trial)
        self.log2txt(results)
        results = 'test Acc={} F1={}'.format(ssacc, ssf1)
        self.log2txt(results)
        results = 'con_matrix2={}'.format(c_m2)
        self.log2txt(results)
        results = 'con_matrix3={}'.format(c_m3)
        self.log2txt(results)
